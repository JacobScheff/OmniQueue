...
export type ModelInfo = {
  ...
  supports_force_any_slot?: boolean;
...
/** A single pinned route position: force this action at this slot. */
export type ForcedPick = {
  slot: number;
  actionId: number;
};
...
export type RecommendResponse = {
  ...
  forced_slot?: number | null;
  forced_action?: number | null;
  route?: RouteStop[];
...
export function fetchWaits(
  force = false,
): Promise<{ rides: WaitRow[]; fetched_at: number; error: string | null }> {
...
export async function postRecommend(
  ...
  forcedPick: ForcedPick | null = null,
  ...
  const res = await fetch("/api/recommend", {
    ...
    body: JSON.stringify({
      ...
      force_action: forcedPick?.actionId ?? null,
    }),
  });
  if (!res.ok) {
